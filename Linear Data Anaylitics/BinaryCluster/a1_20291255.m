function set12 = a1_20291255(elist) 
...
%     set12 - Nx1 vertex clsutering, -1 for SET1 and +1 for SET2
    ...
    % %
    A = zeros(n); %Initialize the empty ajacency matrix
    M = elist; %Rename to fit with other naming practices
    for idx = 1:size(M, 1); %Iterate over every row of the 
        %I got the values of the vectors by using the matrix linear 
        % indexing that MatLab provides. 
        val1 = M(idx); %Get the first value of the edge vector 
        val2 = M(idx + size(M,1));%Get the second value of the edge vector
        A(val1,val2) = 1;%Set the corresponding entry to 1 to 
        % indicate adjacency. 
    end

    dialgVec = sum(A,2); %Sum up the rows of the array to 
    % get the degrees of all the vertexes, 
    D = diag(dialgVec); %Create a matrix with dialgVec as its diagonal
    L = D - A; %Creat the laplacian matrix using A and D
    ...
    [eigVec, ~] = eig(L); %Calculate Eigenvalues
    fiedlerBoy = eigVec(:,2);%Get the 2nd Eigenvactor, the Fielder Vector
    set12 = fiedlerBoy >= 0; %Numbers greater than 0 become 1, 
    set12 = (set12 * 2) - 1; %Makes it so if its not 1, its -1
    ...
    % %
    disp("Set 1:")
    disp(set12 >= 0)%If the feidler value is positive its in set 1. 
    disp("Set 2:")
    disp(set12 < 0)%If the feidler value is negative its in set 1
