...
classify (Atom x) = Fact
classify (Implies question (Atom x)) = if conjunction question == Fact then Rule else NotClause
classify _ = NotClause

conjunction :: Formula -> FormulaKind
conjunction (Atom x) = Fact
conjunction (And (x) (y)) = if conjunction x == Fact then 
                            if conjunction y == Fact then Fact
                            else NotClause
                            else NotClause
conjunction _ = NotClause
...
test_classify2 = (classify (Top) == NotClause)
test_classify3 = (classify (Bot) == NotClause)
test_classify4 = (classify (And (Atom "T") (Atom "T")) == NotClause)
test_classify5 = (classify (Or (Atom "T") (Atom "T")) == NotClause)
test_classify6 = (classify (Not (Atom "T")) == NotClause)
test_classify7 = (classify (Implies (And(Atom "T") (Atom "T")) (Atom "T")) == Rule)
test_classify8 = (classify (Implies (And(Atom "T") (Bot)) (Atom "T")) == NotClause)
test_classify9 = (classify (Implies (And(Bot) (Atom "T")) (Atom "T")) == NotClause)



test_classify :: Bool
test_classify = all (\x -> x) [test_classify1, test_classify2, test_classify3, test_classify4, test_classify5, test_classify6, test_classify7, test_classify8, test_classify9]
...
To build a truth table for a formula, there are 4 steps:
  ...
  for the above example, (And (Atom "p") ("Atom "q")) would result in "p" "q" 
...
getValuations []       = [[]]
getValuations (c : cs) = (map (\ys -> (c,True) : ys) (getValuations cs))  ++  (map (\ys -> (c,False) : ys) (getValuations cs))
...
evalF valu formula =
    case formula of
        ...
        Atom c            -> checkVal valu c
        And phi1 phi2     -> (evalF valu phi1) && (evalF valu phi2)
        Or phi1 phi2      -> (evalF valu phi1) || (evalF valu phi2)

checkVal :: Valuation -> [Char] -> Bool
checkVal ((y,z):ys) x = if x == y && z then True else checkVal ys x
checkVal [] _ = False
...
should be returned by
   ...
   prove_all [vB] (Or vA vdsaidnsiandiasB)
...
prove_core ctx goal (kSucceed, kFail) =
   let   try_prove_left () = prove_left ctx ([], ctx) goal (kSucceed, kFail)
         prove_right_or phi2 () = prove_core ctx phi2 ((\deriv -> \more -> kSucceed (OrRight 2 deriv) more), try_prove_left)
         try_right_rules () =
           case goal of
                Top              -> -- Top-Right rule
                ...
                And phi1 phi2    -> -- And-Right rule
                     prove_core ctx phi1 (\deriv1 -> \more1 -> prove_core ctx phi2 (\deriv2 -> \more2 -> kSucceed (AndRight deriv1 deriv2) more2, more1), try_prove_left)

                Or phi1 phi2     -> -- Or-Right rules (try Or-Right-1, if it fails, try Or-Right-2)
                    prove_core ctx phi1 ((\deriv1 -> \more1 -> (kSucceed (OrRight 1 deriv1) more1)), prove_right_or phi2)
    ...
    in
      case focus of
        ...
        And phi1 phi2 ->      -- And-Left rule
            prove_core (done ++ [phi1] ++ [phi2] ++ rest) goal ((-- kSucceed:
                                                                     \deriv1 -> \more1 -> kSucceed (AndLeft deriv1) more1),
                                                               -- kFail:
                                                                     leave_alone)
 
        
        Or phi1 phi2 ->       -- Or-Left rule
           prove_core (done++[phi1]++rest) goal (\deriv1 -> \more1 -> prove_core (done++[phi2]++rest) goal (\deriv2 -> \more2 -> kSucceed (OrLeft deriv1 deriv2) more2, more1), leave_alone)
